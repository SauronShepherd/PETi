...
object PlatformAppServicesImpl {
    @JvmStatic fun create(context: Context): AppServices = object : AppServices { override val auth: AuthRepository = PersistentLocalAuthRepository(context); override val species: SpeciesRepository = FakeSpeciesRepository(); override val pets: PetRepository = FakePetRepository(); override val analysis = FakeAnalysisRepository(); override val funding: FundingRepository = FakeFundingRepository(); override val mediaUpload = MediaUploadCoordinator(FakeMediaUploadRepository(), SharedPreferencesUploadTaskStore(context), context); override val phase6 = LocalPhase6Repository(); override val records = LocalRecordsRepository(context); override val specialists = LocalSpecialistRepository(); override val reports = LocalReportsRepository(); override val future = LocalFutureRepository() }
}
