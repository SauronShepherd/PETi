...
object PlatformAppServicesImpl {
    @JvmStatic fun create(context: Context): AppServices {
        ...
        return object : AppServices { override val auth = auth; override val species: SpeciesRepository = api; override val pets: PetRepository = api; override val analysis: AnalysisRepository = api; override val funding: FundingRepository = ApiFundingRepository(AppConfig.apiBaseUrl, auth); override val mediaUpload = MediaUploadCoordinator(ApiMediaUploadRepository(AppConfig.apiBaseUrl, auth, context.contentResolver), SharedPreferencesUploadTaskStore(context), context); override val phase6 = CachedPhase6Repository(ApiPhase6Repository(AppConfig.apiBaseUrl, auth), auth, context); override val records = ApiRecordsRepository(AppConfig.apiBaseUrl, auth); override val specialists = ApiSpecialistRepository(AppConfig.apiBaseUrl, auth); override val reports = ApiReportsRepository(AppConfig.apiBaseUrl, auth); override val future = ApiFutureRepository(AppConfig.apiBaseUrl, auth) }
    }
}
